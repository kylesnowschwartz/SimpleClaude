...
#
# This is an EXPERIMENT to see if hooks can improve skill discovery by analyzing
# user prompts and matching them against available skill descriptions.
...
class TriggerSkillsHandler < ClaudeHooks::UserPromptSubmit
  ...
  def extract_triggers(description)
    ...
    # Extract quoted phrases (trigger phrases are often quoted in descriptions)
    description.scan(/"([^"]+)"/).flatten.each do |phrase|
      triggers << phrase.downcase
    end

    # Also extract key action words/phrases
    action_patterns = [
      /(?:when|if).*?(?:asks? to|wants? to|needs? to)\s+["']?([^"',]+)["']?/i,
      /(?:use (?:this|for))\s+(?:when|to)\s+([^.]+)/i
    ]

    action_patterns.each do |pattern|
      description.scan(pattern).flatten.each do |match|
        triggers << match.downcase.strip
      end
    end
  ...
  def calculate_match_score(prompt_words, full_prompt, skill)
    ...
    # Check trigger phrase matches (highest weight)
    skill[:triggers].each do |trigger|
      if full_prompt.include?(trigger)
        # Exact phrase match
        score += 5
      else
        ...
        score += overlap * 2 if overlap > 0
      end
    end

    # Check description word overlap (lower weight)
